...
class VideoPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, F, C, H, W = x.shape
        assert F <= self.num_frames, f'Input tensor has {F} frames, expected {self.num_frames}'
        # NOTE: change to reshape
        x = x.reshape(-1, C, H, W)
        x = self.proj(x)
        return x
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # NOTE: removed the head here, as we want to return the features
        # Classifier head
    ...
    def forward_features(self, x):
        ...
        # NOTE: move this part to EgoVLPv2 (under __init__.py)
        # x = self.norm(x)[:, 0]
        # x = self.pre_logits(x)
        return x[:, 0]
    ...
    def forward(self, x):
        ...
        # NOTE: removed the head since we only want to return the features
        # x = self.head(x)
        return x
